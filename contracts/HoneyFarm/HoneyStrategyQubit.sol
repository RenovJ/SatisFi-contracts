...
// File: contracts/interfaces/IQubitFarm.sol


pragma solidity 0.6.12;

interface IQubitFarm {
    // Deposit tokens to MasterChef.
    function supply(address qToken, uint uAmount) external;

    // Withdraw tokens from MasterChef.
    function redeemToken(address qToken, uint qAmount) external;
...
// File: contracts/StrategyChef.sol
...
contract StrategyChef is Ownable, ReentrancyGuard, Pausable {
    ...
    function _farm(uint256 _wantAmt) internal {
        ...
        IQubitFarm(farmContractAddress).supply(qTokenAddress, _wantAmt);
    }
        ...
        returns (uint256)
    {
        ...
        if (isCompound) {
            IQubitFarm(farmContractAddress).redeemToken(qTokenAddress, _wantAmt);
        }
    ...
    function harvest() public whenNotPaused {
        ...
        // Harvest farm tokens
        IQubitFarm(farmContractAddress).claimQubit();
        distributeFee();
    }
