...
// File: contracts/InitialHoneyOffering.sol
...
/**
 * @dev HoneyFarm: Initial Honey Offering
 *
 * Website: https://honeyfarm.finance
 * Twitter: https://twitter.com/HoneyFarmFi
 *
 */
contract InitialHoneyOffering is ReentrancyGuard, Initializable {
  using SafeMath for uint256;
  ...
  // The block number when IHO starts
  uint256 public startBlock;
  // The block number when IHO ends
  ...
  function deposit(uint256 _amount) public {
    require (block.number > startBlock && block.number < endBlock, 'not IHO time');
  ...
  function finalWithdraw(uint256 _lpAmount, uint256 _offerAmount) public onlyAdmin {
    require (_lpAmount <= lpToken.balanceOf(address(this)), 'not enough raising token');
