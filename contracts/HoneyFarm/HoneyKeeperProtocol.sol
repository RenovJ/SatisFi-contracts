...
contract HoneyKeeperProtocol is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    IBEP20 public buybackToken;  // BUSD
    IBEP20 public sellToken;     // Gokgweingi
    ...
    uint256 public reapplyFeeAmount = 0;
    
    // 12 hr = 43200 sec = 14400 blocks
    uint16 public constant EPOCH_PERIOD_IN_SEC = 300;
    ...
    // newAmount is the amount in sellToken of fee for reapply
    event ReapplyFeeUpdated(uint256 newAmount);
    // newAmount is the amount in sellToken of fee for apply
    event ApplyFeeUpdated(uint256 newAmount);
    // The buyback protocol is ended
    event FinishBuyback();
    ...
    constructor(
        ...
        reapplyFeeAmount = _reapplyFeeAmount;
    ...
    function setBuybackPrice(uint256 _price) public onlyOwner {
        require(_price > 0, "setBuybackPrice: The new buyback price must be higher than 0");
        
        buybackPrice = _price;
        emit BuybackPriceUpdated(_price);
    }
    
    function setBuybackAmountForEachEpoch(uint256 _amount) public onlyOwner {
        require(_amount > 0, "setBuybackAmountForEachEpoch: The new buyback amount for each epoch must be higher than 0");
        
        epochBuybackAmount = _amount;
        emit BuybackAmountForEachEpochUpdated(_amount);
    }

    function setApplyFee(uint256 _amount) public onlyOwner {
        require(_amount > 0, "setApplyFee: The apply fee amount must be higher than 0");
        applyFeeAmount = _amount;
        emit ApplyFeeUpdated(_amount);
    }

    function setReapplyFee(uint256 _amount) public onlyOwner {
        require(_amount > 0, "setReapplyFee: The reapply fee amount must be higher than 0");
        reapplyFeeAmount = _amount;
        emit ReapplyFeeUpdated(_amount);
    }
        
    function finishBuyback() public onlyOwner {
        uint256 balanceBuybackToken = IBEP20(buybackToken).balanceOf(address(this));
        require(balanceBuybackToken > 0, "finishBuyback: no balance to withdraw");
        buybackToken.safeTransfer(address(msg.sender), balanceBuybackToken);
        emit FinishBuyback();
    }
    ...
    function applyBuyback(uint256 _amount) public nonReentrant {
        ...
        uint256 balanceSellTokenBefore = IBEP20(sellToken).balanceOf(address(this));
        ...
        uint256 balanceSellTokenAfter = IBEP20(sellToken).balanceOf(address(this));
        uint256 actualAddedAmount = balanceSellTokenAfter.sub(balanceSellTokenBefore);
        
        applyList[currentEpoch].push(msg.sender);
        userApplyAmount[msg.sender][currentEpoch] = userApplyAmount[msg.sender][currentEpoch].add(actualAddedAmount);
        emit Apply(msg.sender, actualAddedAmount, false);
    }
    
    function reapplyBuyback(uint256 _amount) public nonReentrant {
        uint256 withdrawableBalance = gatherWithdrawableTokenToSell(address(msg.sender));
        require(withdrawableBalance >= _amount, "reapplyBuyback: Not enough balance to reapply");
        require(_amount > reapplyFeeAmount, "reapplyBuyback: The amount to reapply must be larger than reapply fee");
        uint256 reapplyAmount = _amount.sub(reapplyFeeAmount);
        applyList[currentEpoch].push(msg.sender);
        userApplyAmount[msg.sender][currentEpoch] = userApplyAmount[msg.sender][currentEpoch].add(reapplyAmount);
        sellToken.safeTransferFrom(address(this), address(BURN_ADDRESS), reapplyFeeAmount);
        emit Apply(msg.sender, reapplyAmount, true);
    }
