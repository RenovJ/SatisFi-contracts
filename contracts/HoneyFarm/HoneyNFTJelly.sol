...
// File: contracts/HoneyRoyalJelly.sol

contract HoneyNFTJelly is Ownable, ReentrancyGuard, VRFConsumerBase {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        uint256 totalShareAmount; // Total amount of share reserved to the user.
    }
    
    // VRFConsumerBase
    bytes32 internal vrfKeyHash;
    uint256 internal vrfFee;
    uint256 public vrfRandomResult;
        ...
        )
    {
        ...
        vrfFee = 0.2 * 10 ** 18; // 0.1 LINK (Varies by network)
    ...
    function initialize(
        ...
        rewardPerBlock = uint256(10 ** 18).mul(100).div(_bonusEndBlock - _startBlock);
        ...
        PRECISION_FACTOR = uint256(10 ** (uint256(30).sub(18)));
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                user.totalShareAmount = user.totalShareAmount.add(pending);
            }
        }
