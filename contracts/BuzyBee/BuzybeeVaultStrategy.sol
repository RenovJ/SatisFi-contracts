...
contract VaultStrategyBiswap is StratManager, FeeManager {
    ...
    address constant public output = address(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82);
    address public want;
    ...
    constructor(
        ...
        address _strategist
        // address _operator
    //) StratManager(_keeper, _strategist, _unirouter, _vault, _operator) public {
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (tx.origin == owner() || paused()) {
            ...
            IERC20(want).safeTransfer(strategist, withdrawalFeeAmount);
        }
        compound();
    }
    ...
    function chargeFees() internal {
        ...
        // uint256 strategistFee = wbnbBal.mul(strategyFee).div(FEE_MAX);
        // IERC20(wbnb).safeTransfer(strategist, strategistFee);
        uint256 fee = callFee + strategyFee + buybackFee;
        uint256 outputAmount = IERC20(output).balanceOf(address(this));
        IERC20(output).safeTransfer(strategist, outputAmount.mul(fee).div(FEE_MAX));
    }
    ...
    function targetSupplyAmountPerBlock() public view returns (uint256) {
        return IMasterChefReferrer(masterchef).cakePerBlock();
    }
