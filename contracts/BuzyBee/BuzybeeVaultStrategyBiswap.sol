...
contract VaultStrategyBiswap is StratManager, FeeManager {
    ...
    address constant public output = address(0x965F527D9159dCe6288a2219DB51fc6Eef120dD1);
    address public want;
    address public lpToken0;
    address public lpToken1;

    // Third party contracts
    address constant public masterchef = address(0xDbc1A13490deeF9c3C12b44FE77b503c1B061739);
    address constant public bswUnirouter = address(0x3a6d8cA21D1CF76F653A67577FA0D27453350dD8);
    ...
    ) StratManager(_keeper, _feeAddr, bswUnirouter, _vault) public {
        want = _want;
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            IMasterChefReferrer(masterchef).deposit(poolId, wantBal);
        }
        compound();
    }
    ...
    function compound() public whenNotPaused {
        IMasterChefReferrer(masterchef).deposit(poolId, 0);
    ...
    function chargeFees() internal {
        ...
        // uint256 callFeeAmount = wbnbBal.mul(callFee).div(FEE_MAX);
        // IERC20(wbnb).safeTransfer(operator, callFeeAmount);

        // uint256 feeAddrFee = wbnbBal.mul(strategyFee).div(FEE_MAX);
        // IERC20(wbnb).safeTransfer(feeAddr, feeAddrFee);
    ...
    function targetSupplyAmountPerBlock() public view returns (uint256) {
        return IMasterChefReferrer(masterchef).BSWPerBlock();
    }
    
    function targetPoolInfo() external view returns (IERC20, uint256, uint256, uint256) {
